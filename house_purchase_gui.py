...
def calculate_house_budget(*args):
    try:
        ...
        house_budget.set(f'£{"{:,}".format((save_1 + save_2 + equit + mort)-fee)}') 
    except ValueError:
        pass
...
def calulate_total_deposit(*args):
    try:
        ...
        save_2 = int(savings_2.get())
        equit = int(equity.get())
        deposit_total.set(f'£{"{:,}".format(save_1 + save_2 + equit)}')
    except ValueError:
        pass
...
def calculate_amount_to_save(*args):
    try:
        ...
        if int(goal.get()) > current_house_budget:
            total_to_save.set(f'£{"{:,}".format(int(goal.get()) - current_house_budget)}')
        else:
            total_to_save.set("£0")
    except ValueError:
        pass
...
# Create Buttons
ttk.Button(mainframe,text="Show house budget", command=calculate_house_budget).grid(column=1, row=7, sticky=W)
ttk.Button(mainframe, text="Show deposit", command=calulate_total_deposit).grid(column=1, row=8, sticky=W)
ttk.Button(mainframe, text="Show amount to save", command=calculate_amount_to_save).grid(column=1, row=9, sticky=W)


# Create output labels 
house_budget = StringVar()
ttk.Label(mainframe, relief='sunken', textvariable=house_budget).grid(column=2, row=7, sticky=(W, E))

deposit_total = StringVar()
ttk.Label(mainframe,relief='sunken', textvariable=deposit_total).grid(column=2, row=8, sticky=(W, E))

total_to_save = StringVar()
ttk.Label(mainframe,relief='sunken', textvariable=total_to_save).grid(column=2, row=9, sticky=(W, E), )
